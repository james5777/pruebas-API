...
for partner_name, account_id in advertisers.items():
    print(f"📊 Descargando datos de: {partner_name}...")
    ...
    if response.status_code == 200:
        ...
        if not df.empty:
            df["partner"] = partner_name  # 👈 Agregar columna con el nombre del anunciante
            all_data.append(df)
        print(f"✅ Datos obtenidos de {partner_name}")
    else:
        print(f"❌ Error con {partner_name}: {response.status_code} - {response.text}")
...
df_taboola.sort_values(by=["date", "partner"], inplace=True, ignore_index=True)

# ------- Se convierte el dataframe a CSV ------- #
df_taboola.to_csv(name_csv_taboola, index=False, encoding="utf-8-sig")
